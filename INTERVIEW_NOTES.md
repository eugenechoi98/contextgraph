...
## 2026-06-08 - 为什么最后默认还是不开启向量模型
可以这样说：

> 这轮做了两件事：第一，选出了本机现在最合适的代码向量模型；第二，继续把项目默认启动模式保持成无模型。这个设置是用来保证第一次跑项目的人，不会一上来就被模型下载、CPU 开销或者 `trust_remote_code` 这些额外条件卡住。这样做是因为 CodeRankEmbed 现在虽然是推荐方案，但它应该是“明确打开再用”，不是“项目默认偷偷启用”。

## 2026-06-08 - 为什么 Phase 5A 先冻结功能再做开源准备
可以这样说：

> 项目已经有一条能跑通的主链路，这时候最怕继续加功能，把 README、CI、安装验证和安全边界都落在后面。Phase 5A 先冻结功能，是为了把“现在真的能用什么、默认不会做什么、怎么干净安装和测试”讲清楚。这样放到 GitHub 时，别人不会被内部阶段记录、模型下载、缓存文件或本机路径坑住。

## 2026-06-08 - 为什么 clean install smoke 要用仓库外新虚拟环境
可以这样说：
